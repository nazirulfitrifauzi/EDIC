...
class MobileController extends Controller
{
    ...
    public function storeNegeri(Request $request)
    {
        ...
        Session::flash('popupNegeri', 'false');
    ...
    public function storePinjaman(Request $request)
    {
        if (is_null(auth()->user()->pinjaman)) { // xde pinjaman
            $this->validate($request, [
                ...
                // "doc_extra"             => ['file', 'mimes:pdf', 'max:3000'],
            ]);
        } else { // ade pinjaman
            $this->validate($request, [
                // 'purchase_price'        => ['required', 'numeric'],
                ...
                // 'reference_relation'    => ['required', 'string'],
                // 'reference_phone'       => ['required', 'numeric', 'min:10'],
                // "gambar"                => ['file', 'mimes:jpeg,jpg,png', 'max:3000', Rule::requiredIf($request->user()->pinjaman->gambar == NULL)],
        ...
        if (is_null(auth()->user()->pinjaman)) { // pinjaman null
            ...
            // if($request->has('doc_extra')) {
            //     $extra = $request->file('doc_extra');
            //     $extra_name = auth()->user()->ic_no . '_extra.' . $extra->getClientOriginalExtension();
            //     Storage::disk('custom')->putFileAs('/' . $ic_no, $extra, $extra_name);
            // }
            ...
            // if (is_null(auth()->user()->pinjaman->gambar)) { // xde data gambar
            //     $gambar = $request->file('gambar');
            //     $gambar_name = auth()->user()->ic_no . '_gambar.' . $gambar->getClientOriginalExtension();
            //     Storage::disk('custom')->putFileAs('/' . $ic_no, $gambar, $gambar_name);
            // } else { // ade rekod
            ...
            // if (is_null(auth()->user()->pinjaman->document_extra) && $request->has('doc_extra')) {
            //     $extra = $request->file('doc_extra');
            //     $extra_name = auth()->user()->ic_no . '_extra.' . $extra->getClientOriginalExtension();
            //     Storage::disk('custom')->putFileAs('/' . $ic_no, $extra, $extra_name);
            // } else {
            //     $extra_name = auth()->user()->pinjaman->document_extra;
            // }
    ...
    {
        $id = auth()->user()->id;
        $file = Pinjaman::where('user_id', $id)->value('document_extra');

        Pinjaman::where('user_id', $id)->update(['document_extra' => NULL]);
        unlink(public_path('storage/' . auth()->user()->ic_no . '/' . $file));

        User::where('id', $id)->update(['completed' => 0]);
        Pinjaman::where('user_id', $id)->update(['completed' => 0]);

        Session::flash('success', 'Fail Lampiran Tambahan telah dipadam');
        Session::flash('Tab', 'tab3');
    }
