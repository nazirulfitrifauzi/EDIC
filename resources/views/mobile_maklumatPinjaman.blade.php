...
<script>
    $("#extra-div").click(function (event) {
        if (!$(event.target).is('#extra')) {
            $(this).find("#extra").trigger('click');
        }
    });

    $("input[id='extra']").on('change', function () {
        extrareadURL(this);
        extracheckFiles();
    });

    var extracheckFiles = function () {
        if (document.getElementById("extra").files.length > 0) {
            $('#extra-uploaded-div').css('display', 'block');
            $('#extra-div').css('display', 'none');
        } else {
            $('#extra-uploaded-div').css('display', 'none');
            $('#extra-div').css('display', 'block');
        }
    }

    var extrareadURL = function (input) {
        var fullPath = document.getElementById('extra').value;
        if (fullPath) {
            var startIndex = (fullPath.indexOf('\\') >= 0 ? fullPath.lastIndexOf('\\') : fullPath.lastIndexOf('/'));
            var filename = fullPath.substring(startIndex);
            if (filename.indexOf('\\') === 0 || filename.indexOf('/') === 0) {
            ...
            document.getElementById("extra-uploaded").textContent = filename;
        }
    }

    function extraDelFile() {
        $("#extra").val('');
        $('#extra-uploaded-div').css('display', 'none');
        $('#extra-div').css('display', 'block');
    }
