<div class="mt-6">
    ...
    <div id="extra-div"
        class="mt-2 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 @error('doc_extra') border-red-500 @enderror border-dashed rounded-md cursor-pointer"
        style="display: block;">
        @if(isset(auth()->user()->pinjaman->document_extra))
        <div class="flex" x-data="{ open: false }">
            <div class="justify-center">
                <span class="inline-flex rounded-md shadow-sm">
                    <a href="{{ asset('storage/'.auth()->user()->ic_no. '/' . auth()->user()->pinjaman->document_extra) }}"
                    ...
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            <h3 class="text-lg leading-6 font-medium text-gray-900">
                                Padam Fail Lampiran Tambahan!
                            </h3>
                            <div class="mt-2">
                                <p class="text-sm leading-5 text-gray-500">
                                    Adakah anda pasti untuk memadam fail Lampiran Tambahan ini?
                                </p>
                            </div>
                        </div>
                    </div>
                    <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                        <span class="flex w-full rounded-md shadow-sm sm:ml-3 sm:w-auto">
                            <button type="button"
                                ...
                                onclick="event.preventDefault();deleteExtra({{auth()->user()->pinjaman->id}})">
                                Padam!
                            </button>
                        </span>
        ...
        <div class="text-center">
            <input type="file" name="doc_extra" id="extra" class="hidden" />
    ...
    <div id="extra-uploaded-div"
        ...
        <span class="mt-3 inline-flex rounded-md shadow-sm">
            <a id="extra-buttonDel" type="button"
                class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-red-600 hover:bg-red-500 focus:outline-none focus:border-red-700 focus:shadow-outline-red active:bg-red-700 transition ease-in-out duration-150"
                onclick="extraDelFile()">
                <svg fill="currentColor" viewBox="0 0 20 20" class="w-8 h-8">
                    <path fill-rule="evenodd"
        ...
        <span id="extra-uploaded">Tiada fail diupload.</span>
    </div>
    {{-- end test --}}
</div>